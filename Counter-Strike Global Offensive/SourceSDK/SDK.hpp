...
enum EMaterialPropertyTypes : int
{
...
using VertexFormat_t = unsigned long long;
using IntRect = int[ 4 ];
#pragma endregion
...
public:
	...
	QAngle	m_angShutterOpenAngles;
	Vector	m_vecShutterClosePosition;
	QAngle	m_angShutterCloseAngles;
...
public:
	...
	virtual bool GetPropertyFlag( EMaterialPropertyTypes type ) = 0;
...
public:
	void SetValue( float flValue );
	void SetVectorValue( float flX, float flY, float flZ );
	void SetVecComponentValue( float flValue, int iComp );
};

class IMaterialSystem
{
public:
	// TODO : make it on step "chams"
